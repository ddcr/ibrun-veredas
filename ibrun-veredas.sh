...
Usage: $cmd [-np|--ntasks <value>] --mpi=<name> [<opt-affinity-script>] <my_mpiexe> [arg1] ...
  ...
  <name>
                MPI stack [impi_hydra|mvapich2_ssh|mvapich2_slum|openmpi|orca]
  <my_mpiexe>
...
Options:
  ...
  -k,  --keep            Do not delete the hostfile generated by slurm
...
while [ $# -gt 0 ]; do
  ...
  if [ -n "$arg" ]; then
    case "$arg" in
      ...
      -n  | --dryrun  ) dry_run=1;;
      -k  | --keep    ) [[ "$VEREDAS_KEEP_FILES" == "0" ]] && VEREDAS_KEEP_FILES=1;;
      -*              )
...
if [ x"$MPI_MODE" == "xopenmpi" ]; then
  ...
  if [ x"$MPI_MODE" == "xmvapich2_slurm" ]; then
    ...
    std_print "Setting up parallel environment for OpenMPI"
  elif [ x"$MPI_MODE" == "xorcampi" ]; then
    std_print "Setting up parallel environment for ORCA DFT using OpenMPI"
  else
...
if [ x"$MPI_MODE" == "xmvapich2_ssh" ]; then
  ...
  veredas_ompi_options="--mca btl sm,openib,self"

  # check available OpenIB params: ompi_info --param btl openib --level 9
  veredas_ompi_options="${veredas_ompi_options} --mca btl_openib_ib_timeout 23"
  #  veredas_ompi_options="${veredas_ompi_options} --mca btl_openib_use_srq 1"
  #  veredas_ompi_options="${veredas_ompi_options} --mca btl_openib_use_rd_max 2048"

  # check available TCP params: ompi_info --param btl openib --level 9
  veredas_ompi_options="${veredas_ompi_options} --mca oob_tcp_if_include ib0"
  launch_command="mpirun -np $MPI_NSLOTS -hostfile ${hostfile_veredas} ${veredas_ompi_options} $MY_MPIRUN_OPTIONS $fullcmd"
  check_parallel_launcher mpirun
  std_print "$MPI_MODE launch command:"
  std_print "          $launch_command"
  if [ "$dry_run" -eq "0" ]; then
    mpirun -np $MPI_NSLOTS -hostfile ${hostfile_veredas} ${veredas_ompi_options} $MY_MPIRUN_OPTIONS $fullcmd
    res=$?
  else
    res=0
  fi
  ...
  # check available TCP params: ompi_info --param btl openib --level 9
  veredas_ompi_options="${veredas_ompi_options} --mca oob_tcp_if_include ib0"
  launch_command="orca $fullcmd \"-machinefile ${hostfile_veredas} ${veredas_ompi_options} $MY_MPIRUN_OPTIONS\""

  if [ ! $(type -P "mpirun") ]; then
      err_print "Cannot find launcher ORCA DFT without OpenMPI modulefile loaded"
      exit 1;
  fi
  check_parallel_launcher orca

  std_print "$MPI_MODE launch command:"
  std_print "          $launch_command"
  if [ "$dry_run" -eq "0" ]; then
    orca $fullcmd "-machinefile ${hostfile_veredas} ${veredas_ompi_options} $MY_MPIRUN_OPTIONS"
    res=$?
  else
    res=0
  fi
